...
from agentflow.algorithms.context_resolver.vocabulary import find_lock_conflicts
...
def apply_context_budget(
    ...
    *,
    ...
    final: dict[str, str] = {"lock_identity": raw["lock_identity"]}
    effective_total = max(0, total_prompt_budget - SEGMENT_SEPARATOR_RESERVE)
    visible_allow = max(VISIBLE_PROMPT_FLOOR, effective_total - len(final["lock_identity"]))
    final["visible_prompt"] = _truncate(raw["visible_prompt"], visible_allow)

    remaining = max(0, effective_total - len(final["lock_identity"]) - len(final["visible_prompt"]))
