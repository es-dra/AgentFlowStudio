    ...
    *,
    ...
    return "\n".join(
        [
            ...
            f"角色/主体：{subject}。保持首帧中的角色身份、脸部辨识度、服装、发型轮廓、体态比例和画风一致。",
...
def video_enhancement_instruction(
    *,
    ...
    reference_mode = str(
        params.get("reference_transform_mode")
        or params.get("referenceTransformMode")
        or params.get("asset_reference_mode")
        or "",
    ).strip().lower()
    originalize = reference_mode in {"originalize", "originalize_ip_safe", "ip_safe_rebirth", "ip_risk_reduction"}
    first_frame_line = (
    ...
    if originalize:
        parts = [
            f"意图：基于首帧/参考图的灵感生成视频，围绕“{prompt_text}”组织一个原创视频段落，降低可识别 IP 相似度。",
            "角色/主体：不复刻首帧身份、脸部、服装、标志物或精确比例；只保留宽泛类别、材质情绪和功能方向，重新设计可动画主体。",
            "场景/美术：借鉴氛围和色彩关系即可，重新组织场景、道具和空间层次，避免延续经典场景或 franchise 视觉符号。",
            f"动作/情节：从新设计的首帧自然开始运动；{('用户运动要求：' + motion) if motion else '设计轻微、自然、克制但明确的动作。'}",
            "镜头/构图：重新设计镜头运动和主体位置关系，避免复刻参考图的标志性姿势、剪影或构图。",
            "灯光：保持视频可读性，重新设计光源方向、曝光和色温，避免形成原图复刻感。",
            "运动/时间推进：写清楚时间推进、动作方向、速度、幅度和镜头关系；必须是连续视频段落。",
            "连续性：保持新原创主体、场景和光线在视频内部一致；参考图只作为灵感，不作为身份或构图锁。",
            "负面约束：不要复制已知角色、商标、logo、标志性服装/武器/姿势/构图，不要静止不动、身份漂移、肢体畸形、文字水印、突兀转场或背景跳变。",
        ]
    else:
        parts = [
            f"意图：基于首帧生成视频，围绕“{prompt_text}”组织连续运动，不写成图生图或单帧编辑。",
            "角色/主体：明确首帧中的主体；保持角色身份、脸部辨识度、服装、发型轮廓、体态比例和整体画风一致。",
    ...
    return "\n".join(
        [
            ...
            "参考图用途：原创重生 / 降 IP 风险，首帧只提供灵感，不作为身份复刻锚点。" if originalize else "",
            "硬性要求：只输出可直接用于图生视频/首帧生视频的中文提示词；不要解释、不输出思考过程、不添加标题。",
            "禁止事项：不要写“本次只做图生图编辑”“单帧图像编辑”“不制造多阶段动作或剧情”；不要把上游节点标题或完整旧提示词当成角色名字。",
