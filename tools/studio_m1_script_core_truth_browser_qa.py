...
def assert_script_core_truth_interaction(page: Page, base_url: str) -> dict[str, Any]:
    ...
    expect(page.locator(".agent-command-preview").filter(has_text="创建剧本版本")).to_be_visible()
    page.get_by_role("button", name="确认执行").click()
    expect(page.locator(".agent-receipt").filter(has_text="剧本版本已创建")).to_be_visible()
    ...
    page.get_by_role("button", name="发送到 Agent Chat").click()
    expect(page.locator(".agent-command-preview").filter(has_text="刷新剧本与资产事实")).to_be_visible()
    page.get_by_role("button", name="确认执行").click()
