...
Status: internal evaluation dimension v0.1 (research / verification-only).

Live observation generation historically used `run_stability_checks.py`, which
imports production `apps.api` modules. That runner is **out of this research
slice**. Checked-in `stability_observations.json` + offline
`score_long_script_stability.py` are enough to verify the checklist protocol.
