# AFS-LOULAN-WEB-ROOT-LATEST-GATE-FACTS-DIRECT-001
...
Add a stable `latest_gate_facts` fact to the directly selected Loulan root
`project_manifest.json` inspector.

The fact resolves the newest available gate-facts status in this order:
...
This avoids adding a new root inspector fact for every future gate-facts sync
object.
