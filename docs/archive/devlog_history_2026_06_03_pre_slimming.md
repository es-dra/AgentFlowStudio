...
- Split the remaining current oversized files without changing behavior:
  ...
  `agentflow_studio/memory_advantage_demo_012_review_html.py`, DEMO-012 test
...
- Tightened `tools/run_memory_advantage_recording_016.ps1`: live mode now
  requires `-ProviderConfig` or `AFS_PROVIDER_CONFIG` in addition to
  `-AllowRemoteVideo` or `AFS_ALLOW_REMOTE_VIDEO=true`.
...
- Added `AFS_PROVIDER_CONFIG` as the environment-variable fallback when
  a provider config path is not passed explicitly.
- Updated hidden provider/operator CLI commands so `--provider-config` defaults
  to `None`, with help text pointing to `AFS_PROVIDER_CONFIG`.
...
- Verification:
  - `.\.venv\Scripts\python.exe -B -m pytest --assert=plain tests/test_kling_video_request_plan.py tests/test_kling_video_smoke.py tests/test_kling_video_t2v_smoke.py tests/test_kling_video_curl_smoke.py tests/test_kling_video_task_recovery.py tests/test_minimax_image_smoke.py -q`
    -> 29 passed.
  - `.\.venv\Scripts\python.exe -B -m py_compile agentflow_studio/model_gateway/company_secrets.py apps/cli/kling_video_command.py apps/cli/minimax_image_command.py apps/cli/memory_demo_commands.py`
    -> passed.
...
- Captured the staging-blocking design issue that
  `agentflow_studio/model_gateway/company_secrets.py` hardcodes a local Company
...
- Remote-provider policy: no remote provider calls. Verification used local
  tests only.
- Extended `agentflow_studio/workflow_engine/node_artifacts.py` with the shared
...
- Reused `node_artifacts.require_input`, `require_output`, and
  ...
  transcription, highlight, OCR, AgentFlow Production, and PosterFlow nodes.
...
- Continued the workflow-engine cleanup under the updated project operating
  ...
  `agentflow_studio.workflow_engine` base node/helper layer and project records.
- Remote-provider policy: no remote provider calls. Verification used local
  tests only.
- Split artifact JSON loading, schema validation, and workflow state fallback
  helpers out of `agentflow_studio/workflow_engine/nodes.py` into
  `agentflow_studio/workflow_engine/node_artifacts.py`.
...
- Verification:
  - `python -m py_compile agentflow_studio/workflow_engine/nodes.py agentflow_studio/workflow_engine/node_artifacts.py`
    -> passed.
...
- Split request planning and safe provider-config resolution into
  `agentflow_studio/model_gateway/minimax_image_plan.py`.
- Split runtime subject-reference validation, prompt-pack construction, and
  image-output summaries into
  `agentflow_studio/model_gateway/minimax_image_runtime.py`.
- Kept public compatibility through
  `agentflow_studio/model_gateway/minimax_image_smoke.py`:
...
- Verification:
  - `pytest tests/test_minimax_image_smoke.py tests/test_memory_advantage_demo_012.py tests/test_posterflow_provider.py -q`
    -> 32 passed.
  - `python -m py_compile agentflow_studio/model_gateway/minimax_image_smoke.py agentflow_studio/model_gateway/minimax_image_plan.py agentflow_studio/model_gateway/minimax_image_runtime.py`
    -> passed.
...
- Split Kling video runtime concerns out of
  `agentflow_studio/model_gateway/kling_video_smoke.py` into
  `agentflow_studio/model_gateway/kling_video_runtime.py`.
...
- Verification:
  - `pytest tests/test_kling_video_smoke.py tests/test_kling_video_t2v_smoke.py tests/test_kling_video_curl_smoke.py tests/test_kling_video_request_plan.py tests/test_kling_video_task_recovery.py -q`
    -> 16 passed.
  - `python -m py_compile agentflow_studio/model_gateway/kling_video_smoke.py agentflow_studio/model_gateway/kling_video_runtime.py tests/test_kling_video_smoke.py tests/test_kling_video_t2v_smoke.py tests/test_kling_video_curl_smoke.py tests/test_kling_video_task_recovery.py tests/test_kling_video_request_plan.py tests/kling_video_smoke_helpers.py`
    -> passed.
