...
Proposal extraction is disabled by default. It runs only when
`AFS_ENABLE_SCENE_NAME_NORMALIZATION_PROPOSALS` is explicitly enabled, and a
submitted proposal is rejected while that gate is off.
...
If the canonical scene is already confirmed, confirmation updates its existing
Production Graph node before Script Truth state is committed. The command is
safe to retry after a partial Script Truth write failure: the Graph mutation is
replayed idempotently and the missing receipt/state projection is recovered.
Undo follows the same Graph-first rule and restores the prior alias set and
asset version on that same node. Candidate scenes do not write Production Graph
until their separate asset review is explicitly confirmed.
