# AgentFlow Production Contracts

AgentFlow Production MVP is a local-first structured production handoff generator.

AgentFlow Studio remains the distribution-side module of AgentFlow Studio.
AgentFlow Production is a sibling module under the `AgentFlowStudio` repository for
MVP validation. The repository container has been renamed, but the
AgentFlow Production package, AgentFlow Studio package, current CLI, workflows, and artifact
...
```text
agentflow_production_brief_to_production_handoff_v0.1
...
```powershell
.venv\Scripts\python.exe -m apps.cli.main run-workflow --workflow workflows/agentflow_production_handoff.yaml --input examples/agentflow_production/creative_brief.example.json --output data/processed/runs/demo_agentflow_production_handoff
```
...
The `agentflow_production_handoff` quality profile checks:
...
project/AgentFlow Production handoff, but strong consistency remains in the JSON
