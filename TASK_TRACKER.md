...
| AFS-RUN-PACKAGE-001 | `codex/afs-run-package-loop` / `C:\Users\chenzy\.config\superpowers\worktrees\AgentFlowStudio\afs-run-package-loop` | Workflow Engineer + Harness / QA Reviewer | Local runtime package evidence or actionable local-input blocker | ready after scenario package integration | Pending | Can run in parallel with `AFS-WEB-OPERATOR-002` |
| AFS-WEB-OPERATOR-002 | `codex/afs-web-operator-loop` / `C:\Users\chenzy\.config\superpowers\worktrees\AgentFlowStudio\afs-web-operator-loop` | Web UI Agent + QA Reviewer | Web operator path for the 0.4 scenario | ready for integration | Web targeted tests -> 45 passed; JS `node --check`; compileall; `git diff --check` | Evidence: `docs/handoff/AFS-WEB-OPERATOR-002.md`; defaults to Local Alpha 0.4 input/output and surfaces local setup blockers; no provider calls |
...
| AFS-RUN-PACKAGE-001 | `codex/afs-run-package-loop` / `C:\Users\chenzy\.config\superpowers\worktrees\AgentFlowStudio\afs-run-package-loop` | Workflow Engineer + Harness / QA Reviewer | local workflow runbook, handoff, focused tests if needed | ready after scenario package integration |
| AFS-WEB-OPERATOR-002 | `codex/afs-web-operator-loop` / `C:\Users\chenzy\.config\superpowers\worktrees\AgentFlowStudio\afs-web-operator-loop` | Web UI Agent + QA Reviewer | `apps/web/`, `apps/web_bridge/`, Web tests/docs | ready for integration; 0.4 defaults and blocker display implemented |
