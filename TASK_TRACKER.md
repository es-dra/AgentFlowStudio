...
| Mainline staging bundle | #73 merged; #72 draft preparing for master review | `docs/maintenance/AFS-MAINLINE-STAGING-BUNDLE-001.md` defines the current integration bundle as product mainline plus reviewed hidden support/evidence; #73 merged the Local Alpha 0.4 base into `master` as `94401afe`, while #72 remains draft and was rebased onto `origin/master` so it can be retargeted for bundle review; CLI registration is split between product `command_registry.py` and hidden `support_command_registry.py`; post-rebase full suite passed (`675 passed`); no provider calls |
