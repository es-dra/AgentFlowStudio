...
| Mainline staging bundle | pushed as draft PR #72; depends on PR #73 ready for review | `docs/maintenance/AFS-MAINLINE-STAGING-BUNDLE-001.md` defines the current integration bundle as product mainline plus reviewed hidden support/evidence; #73 publishes the local Local Alpha 0.4 base that was ahead of `origin/master` and is now ready for review, while #72 remains draft and was rebased onto the updated #73 base (`35bba58`) so it reviews the bundle commits separately; CLI registration is split between product `command_registry.py` and hidden `support_command_registry.py`; post-rebase full suite passed (`675 passed`); no provider calls |
