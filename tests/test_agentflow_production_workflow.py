...
def test_agentflow_production_workflow_generates_production_handoff_artifacts(tmp_path) -> None:
    ...
    assert workflow.name == "agentflow_production_brief_to_production_handoff_v0.1"
    assert workflow.quality_profile == "agentflow_production_handoff"
    ...
    assert run_manifest["module"] == "AgentFlow Production"
...
def test_agentflow_production_workflow_accepts_creative_brief_json_as_cli_input(tmp_path) -> None:
    output_dir = tmp_path / "agentflow_production_cli_run"
...
def test_agentflow_production_inspect_and_review_pass_for_valid_run(tmp_path) -> None:
    ...
    assert inspection["quality_report"]["summary"]["quality_profile"] == "agentflow_production_handoff"
    assert review["status"] == "passed"
    assert "agentflow_production_artifacts" in [section["name"] for section in review["sections"]]


def test_agentflow_production_review_fails_on_prompt_shot_mismatch(tmp_path) -> None:
    output_dir = _run_agentflow_production_workflow(tmp_path)
    ...
    assert "agentflow_production_prompt_shots_exist" in failed_ids


def test_agentflow_production_review_fails_on_artifact_schema_error(tmp_path) -> None:
    output_dir = _run_agentflow_production_workflow(tmp_path)
    ...
    assert review["status"] == "failed"
    assert "agentflow_production_handoff_schema_valid" in failed_ids


def test_agentflow_production_review_fails_on_invalid_json(tmp_path) -> None:
    output_dir = _run_agentflow_production_workflow(tmp_path)
    ...
    assert review["status"] == "failed"
    assert "agentflow_production_prompt_pack_json_valid" in failed_ids
