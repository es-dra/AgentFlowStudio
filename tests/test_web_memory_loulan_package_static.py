...
"""
    ...
    assert "manifest reference: pass (errors 0; invalid types 0; invalid statuses 0)" in payload["bundle"]
    assert "text encoding: pass (errors 0)" in payload["bundle"]
    assert "phase gate: blocked_until_b01_human_review (failures 0)" in payload["bundle"]
    ...
    assert "invalid asset types: 0" in payload["protocol"]
    assert "invalid statuses: 0" in payload["protocol"]
    assert "pending B01: 5" in payload["protocol"]
    ...
    assert "promotion_gate: blocked" in payload["inspector"]
    assert "next_context_status: promotion_decision_required" in payload["inspector"]
    assert "context_rule: only approved_anchor or promoted_reusable assets may enter context" in payload["inspector"]
    assert "b01_validation_status: blocked_pending_human_review" in payload["inspector"]
    assert "b01_apply_status: blocked_validation_not_ready" in payload["inspector"]
    assert "b01_context_projection_ready: false" in payload["inspector"]
    assert "b01_human_acceptance_recorded: false" in payload["inspector"]
    assert "b01_media_generation_started: false" in payload["inspector"]
    assert "b01_operator_apply_status: blocked_validation_not_ready" in payload["inspector"]
    assert "b01_operator_next_context: blocked_until_b01_human_review" in payload["inspector"]
    assert "writes_long_term_memory: false" in payload["inspector"]
    assert "manifest_reference_audit: pass" in payload["inspector"]
    assert "manifest_audit_errors: 0" in payload["inspector"]
    assert "invalid_asset_types: 0" in payload["inspector"]
    assert "invalid_statuses: 0" in payload["inspector"]
    assert "text_encoding_audit: pass" in payload["inspector"]
    assert "text_encoding_errors: 0" in payload["inspector"]
    assert "phase_gate_audit: blocked_until_b01_human_review" in payload["inspector"]
    assert "phase_gate_failures: 0" in payload["inspector"]
