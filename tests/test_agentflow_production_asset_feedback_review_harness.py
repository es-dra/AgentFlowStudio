...
from agentflow.harness.agentflow_production_review import validate_agentflow_production_asset_feedback_review


REVIEW_EXAMPLE = Path("examples/agentflow/agentflow_production_asset_feedback_review.example.json")


def test_agentflow_production_asset_feedback_review_harness_accepts_current_example() -> None:
    review = _review_example()
    original = copy.deepcopy(review)

    validation = validate_agentflow_production_asset_feedback_review(review)

    assert validation["schema_version"] == "0.1.0"
    assert validation["artifact_type"] == "agentflow_production_asset_feedback_review_validation"
    assert validation["validation_scope"] == "agentflow_production_asset_feedback_review"
...
def test_agentflow_production_asset_feedback_review_harness_rejects_runtime_claims() -> None:
    review = _review_example()
...
def test_agentflow_production_asset_feedback_review_harness_requires_step_consistency() -> None:
    review = _review_example()
...
def test_agentflow_production_asset_feedback_review_harness_rejects_private_paths() -> None:
    review = _review_example()
