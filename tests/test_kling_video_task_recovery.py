...
def test_kling_completion_falls_back_from_httpx_to_curl_without_resubmitting(monkeypatch, tmp_path) -> None:
    ...
    monkeypatch.setattr(kling_video_smoke, "download_with_transport", fake_download_with_transport)

    manifest = kling_video_smoke._complete_video_task_with_transport_fallback(
        output_dir,
        state=state,
