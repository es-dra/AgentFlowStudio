...
def test_agentflow_narratostudio_asset_reuse_review_example_is_review_only() -> None:
    payload = json.loads(
        Path("examples/agentflow/narratostudio_asset_reuse_review.example.json").read_text(encoding="utf-8")
    )

    assert payload["schema_version"] == "0.1.0"
    assert payload["artifact_type"] == "agentflow_narratostudio_asset_reuse_review"
    assert payload["review_scope"] == "narratostudio_asset_reuse_dry_run_chain"
    ...
    assert payload["source_artifact_types"]["dry_run_plan"] == "agentflow_narratostudio_asset_reuse_dry_run_plan"
