...
def test_operator_loop_writer_can_emit_handoff_packet_with_manifest_check(tmp_path: Path) -> None:
    output_dir = tmp_path / "operator_loop"
    loop = load_production_memory_loop(EXAMPLE_PATH)
    result = build_production_memory_operator_loop_run(
        loop,
    ...
    written_paths = write_production_memory_operator_loop_run(result, output_dir, write_handoff_packet=True)

    check_path = output_dir / "operator_manifest_check" / "operator_manifest_check.json"
    handoff_path = output_dir / "operator_handoff" / "operator_handoff_packet.json"
    handoff_md_path = output_dir / "operator_handoff" / "operator_handoff_packet.md"
    assert check_path in written_paths
    assert handoff_path in written_paths
    assert handoff_md_path in written_paths
    check = json.loads(check_path.read_text(encoding="utf-8"))
    ...
    assert result["operator_manifest_check"]["check_status"] == "passed"
    assert result["operator_handoff_packet"]["handoff_status"] == "ready"
...
def test_operator_loop_cli_can_write_handoff_packet_with_manifest_check(tmp_path: Path) -> None:
    output_dir = tmp_path / "operator_loop"
    ...
    assert (output_dir / "operator_manifest_check" / "operator_manifest_check.json").exists()
    assert (output_dir / "operator_handoff" / "operator_handoff_packet.json").exists()
    assert (output_dir / "operator_handoff" / "operator_handoff_packet.md").exists()
