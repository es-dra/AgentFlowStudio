...
from agentflow.memory.agentflow_production_assets import build_agentflow_production_asset_memory_contract_set
from agentflow_studio.workflow_engine import WorkflowContext, WorkflowRunner, default_node_registry, load_workflow


def test_agentflow_production_workflow_outputs_build_asset_memory_contract_set(tmp_path: Path) -> None:
    ...
    contracts = build_agentflow_production_asset_memory_contract_set(**source_payloads)
    ...
    assert contracts["intermediate_asset"]["module_origin"] == "AgentFlow Production"
...
def test_agentflow_production_asset_memory_contract_set_requires_candidate_memory() -> None:
    payloads = {
    ...
    with pytest.raises(ValueError, match="memory candidate"):
        build_agentflow_production_asset_memory_contract_set(**payloads)


def test_agentflow_production_asset_memory_contract_set_does_not_normalize_missing_feedback_source(
    tmp_path: Path,
) -> None:
    output_dir = _run_agentflow_production_workflow(tmp_path)
    source_payloads = _source_payloads(output_dir)
...
def test_agentflow_production_asset_memory_contract_set_does_not_normalize_missing_promotion_status(
    tmp_path: Path,
) -> None:
    output_dir = _run_agentflow_production_workflow(tmp_path)
    source_payloads = _source_payloads(output_dir)
