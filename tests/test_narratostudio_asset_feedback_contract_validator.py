...
from agentflow.memory.narratostudio_assets import validate_narratostudio_asset_feedback_sources
from narratocut.workflow_engine import WorkflowContext, WorkflowRunner, default_node_registry, load_workflow


def test_narratostudio_asset_feedback_source_validator_accepts_current_workflow_outputs(tmp_path: Path) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    sources = _source_payloads(output_dir)
    original_sources = copy.deepcopy(sources)

    validation = validate_narratostudio_asset_feedback_sources(**sources)

    assert validation["schema_version"] == "0.1.0"
    assert validation["artifact_type"] == "agentflow_narratostudio_asset_feedback_source_validation"
    assert validation["validation_scope"] == "narratostudio_asset_feedback_sources"
...
def test_narratostudio_asset_feedback_source_validator_rejects_primary_feedback_store(tmp_path: Path) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    sources = _source_payloads(output_dir)
...
def test_narratostudio_asset_feedback_source_validator_rejects_malformed_candidate_store(
    tmp_path: Path,
) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    sources = _source_payloads(output_dir)
...
def test_narratostudio_asset_feedback_source_validator_requires_candidate_identity(
    tmp_path: Path,
) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    sources = _source_payloads(output_dir)
...
def test_narratostudio_asset_feedback_source_validator_rejects_non_local_cost_trace(tmp_path: Path) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    sources = _source_payloads(output_dir)
...
def test_narratostudio_asset_feedback_source_validator_rejects_handoff_without_prompt_pack_ref(
    tmp_path: Path,
) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    sources = _source_payloads(output_dir)
