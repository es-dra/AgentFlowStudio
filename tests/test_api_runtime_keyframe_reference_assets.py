...
def test_initial_asset_card_generation_does_not_send_reference_images(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("AFS_ALLOW_REMOTE_IMAGE", "true")
    captured: dict[str, object] = {}
    ...
    monkeypatch.setattr(
        ...
        lambda: _FakeRegistry(fake_dispatch, _FakeDescriptor(reference_image_slots=4)),
    )
    client = TestClient(create_runtime_app(runtime_root=tmp_path))
    project_id = "proj_initial_asset_card_no_refs"
    asset_id = _upload_reference(client, project_id, "existing-character.png")

    result = client.post(
        f"/projects/{project_id}/keyframe-generations",
        json={
            "node_id": "asset-card-node",
            "prompt_text": "Create a reusable prop reference sheet for a red apple.",
            "optimized_prompt": "Create a reusable prop reference sheet for a red apple.",
            "target_platform": "short_video",
            "style": "cinematic",
            "aspect_ratio": "1:1",
            "candidate_count": 1,
            ...
            "context_subgraph": {
                "target_node_id": "asset-card-node",
                "nodes": [
                    {"id": "script-node", "type": "script", "visual_asset_ids": [asset_id]},
                    {"id": "asset-card-node", "type": "image", "image_asset_refs": [asset_id]},
                ],
                "edges": [{"id": "edge-1", "from": "script-node", "to": "asset-card-node", "relation_type": "generation"}],
            },
            "generated_at": "2026-06-29T18:00:00+08:00",
        },
    )
    ...
    assert captured["reference_paths"] == []
    assert captured["subject_reference_image_path"] is None
    assert captured["image_operation"] == "generate"
    assert plan["reference_image_count"] == 0
    assert plan["reference_images"] == []


def test_initial_asset_card_preflight_omits_reference_image_channel(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("AFS_ALLOW_REMOTE_IMAGE", "true")

    class DescriptorRegistry:
        def descriptor(self, service_id: str):
            return _FakeDescriptor(reference_image_slots=4)

    monkeypatch.setattr("apps.api.runtime_generation_preflight.load_provider_registry", lambda: DescriptorRegistry())
    client = TestClient(create_runtime_app(runtime_root=tmp_path))
    project_id = "proj_initial_asset_card_preflight_no_refs"
    asset_id = _upload_reference(client, project_id, "existing-character.png")
