...
def test_narratostudio_review_fails_when_scene_references_missing_beat(tmp_path) -> None:
    ...
    assert "narratostudio_scene_beats_exist" in failed_ids


def test_narratostudio_review_fails_when_outline_beat_has_no_scene(tmp_path) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    scene_plan = _read_json(output_dir / "scene_plan.json")
    ...
    assert "narratostudio_outline_beats_covered_by_scenes" in failed_ids


def test_narratostudio_review_fails_when_scene_has_no_shot(tmp_path) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    ...
    assert "narratostudio_scenes_covered_by_shots" in failed_ids


def test_narratostudio_review_fails_when_shot_has_no_prompt(tmp_path) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    ...
    assert "narratostudio_shots_covered_by_prompts" in failed_ids


def test_narratostudio_review_fails_when_handoff_core_reference_breaks(tmp_path) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    ...
    assert "narratostudio_handoff_core_ids_match" in failed_ids


def test_narratostudio_review_fails_when_handoff_artifact_ref_missing(tmp_path) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    handoff = _read_json(output_dir / "production_handoff.json")
    ...
    assert "narratostudio_handoff_artifact_refs_complete" in failed_ids


def test_narratostudio_review_lightly_checks_production_report_identity(tmp_path) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    (output_dir / "production_report.md").write_text("unrelated report\n", encoding="utf-8")

    failed_ids = _failed_review_check_ids(output_dir)

    assert "narratostudio_production_report_identity" in failed_ids
