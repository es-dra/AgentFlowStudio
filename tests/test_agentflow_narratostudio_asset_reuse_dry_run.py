...
def test_narratostudio_asset_reuse_dry_run_plan_uses_passed_gate_without_executing() -> None:
    ...
    plan = plan_narratostudio_asset_reuse_dry_run(
        review_gate=gate,
    ...
    assert plan["target_task"] == "narratostudio_brief_to_production_handoff"
...
def test_narratostudio_asset_reuse_dry_run_plan_blocks_when_gate_is_not_passed() -> None:
    gate = _json(GATE_EXAMPLE)
...
def test_narratostudio_asset_reuse_dry_run_plan_blocks_mismatched_profile_selection() -> None:
    reusable_profile = _json(REUSABLE_PROFILE_EXAMPLE)
    reuse_decision = _json(REUSE_DECISION_EXAMPLE)
...
def test_narratostudio_asset_reuse_dry_run_plan_blocks_unprovided_profile_selection() -> None:
    reusable_profile = _json(REUSABLE_PROFILE_EXAMPLE)
    reuse_decision = _json(REUSE_DECISION_EXAMPLE)
...
def test_narratostudio_asset_reuse_dry_run_plan_blocks_missing_profile_id() -> None:
    reusable_profile = _json(REUSABLE_PROFILE_EXAMPLE)
    reusable_profile["asset_profile_id"] = ""
...
def test_narratostudio_asset_reuse_dry_run_plan_rejects_execution_or_memory_write_claims() -> None:
    gate = _json(GATE_EXAMPLE)
