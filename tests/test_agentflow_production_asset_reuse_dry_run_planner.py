...
def test_agentflow_production_asset_reuse_dry_run_planner_accepts_passed_gate() -> None:
    review = _json(REVIEW_EXAMPLE)
    gate = _json(GATE_EXAMPLE)
    original_review = copy.deepcopy(review)
    original_gate = copy.deepcopy(gate)

    plan = plan_agentflow_production_asset_reuse_dry_run(review=review, gate=gate)
...
def test_agentflow_production_asset_reuse_dry_run_planner_blocks_failed_gate() -> None:
    review = _json(REVIEW_EXAMPLE)
    gate = _json(GATE_EXAMPLE)
...
def test_agentflow_production_asset_reuse_dry_run_planner_rejects_runtime_or_memory_claims() -> None:
    review = _json(REVIEW_EXAMPLE)
    gate = _json(GATE_EXAMPLE)
