...
from agentflow.memory.narratostudio_review import review_narratostudio_asset_feedback_loop
from narratocut.workflow_engine import WorkflowContext, WorkflowRunner, default_node_registry, load_workflow


def test_narratostudio_asset_feedback_review_surface_accepts_current_workflow_outputs(
    tmp_path: Path,
) -> None:
    ...
    review = review_narratostudio_asset_feedback_loop(**sources)

    assert review["schema_version"] == "0.1.0"
    assert review["artifact_type"] == "agentflow_narratostudio_asset_feedback_review"
    assert review["validation_scope"] == "narratostudio_asset_feedback_loop"
...
def test_narratostudio_asset_feedback_review_surface_does_not_build_when_sources_fail(
    tmp_path: Path,
) -> None:
    output_dir = _run_narratostudio_workflow(tmp_path)
    sources = _source_payloads(output_dir)
