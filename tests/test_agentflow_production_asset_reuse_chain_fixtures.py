...
def test_agentflow_production_asset_reuse_chain_fixture_builder_returns_ready_chain() -> None:
    ...
    assert chain["validation"]["artifact_type"] == "agentflow_production_asset_feedback_review_validation"
    assert chain["validation"]["overall_status"] == "passed"
    assert chain["gate"]["artifact_type"] == "agentflow_production_asset_feedback_review_gate"
    assert chain["gate"]["gate_status"] == "passed"
    assert chain["dry_run_plan"]["artifact_type"] == "agentflow_production_asset_reuse_dry_run_plan"
    assert chain["dry_run_plan"]["plan_status"] == "ready"
    assert chain["reuse_review"]["artifact_type"] == "agentflow_production_asset_reuse_review"
...
def test_agentflow_production_asset_reuse_chain_fixture_builder_blocks_failed_review() -> None:
    review = _json(REVIEW_EXAMPLE)
...
def test_agentflow_production_asset_reuse_chain_fixture_builder_does_not_write_or_execute() -> None:
    review = _json(REVIEW_EXAMPLE)
