...
from agentflow_studio.package_sop.report_selection import selection_diagnostic_lines
...
def _target_duration(candidate: dict[str, Any] | None) -> str:
    ...
    return _seconds(_float(value)) if value is not None else "not applicable"


def _source_window(candidate: dict[str, Any] | None, fallback_start: float | None, fallback_end: float | None) -> str:
    evidence = _source_evidence(candidate)
    if evidence:
        value = _time_range(_float(evidence.get("source_window_start_sec")), _float(evidence.get("source_window_end_sec")))
        if value != "unknown":
            return value
    if _source_candidate(candidate) is not None:
...
def _source_candidate(candidate: dict[str, Any] | None) -> dict[str, Any] | None:
    if not candidate:
        return None
    source_candidate = candidate.get("source_candidate")
    return source_candidate if isinstance(source_candidate, dict) else None
