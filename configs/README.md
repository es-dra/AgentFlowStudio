...
- `providers.example.json`：新的统一 provider registry 示例，覆盖 LLM、vision、
  image、video、descriptor 和 account pool；默认 LLM/vision 通过服务器本机
  Codex 接入，image/video 通过外部 relay 接入。
...
`codex_local`，`image_relay` 与 `seedance_i2v` 使用外部 relay；账号池只保存环境变量名称，不保存 secret 值。Runtime trace、
