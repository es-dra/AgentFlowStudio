...
- Browser smoke reached `ready_for_next_round` with provider calls still not started; follow-up browser review found old demo text, raw project ids, mojibake titles, and English Runtime projection copy leaking into the first screen, so the visible projection copy and project list were localized.
