...
2026-06-01: Loulan API workbench now accepts optional context bundle projections from explicit human decisions and blocks fallback when supplied projections are not ready; real Loulan no-call context probe confirms the current chain blocks on 47 pending human decisions with final API preview at 0 requests; see `docs/handoff/AFS-LOULAN-API-CONTEXT-001.md` and `docs/handoff/AFS-LOULAN-CONTEXT-PROBE-001.md`.
