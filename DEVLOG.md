...
2026-06-01: Loulan root `project_manifest.json` selected-file inspector now prefers the project-audit latest gate-facts status when resolving `latest_gate_facts`, keeping the root project entry point aligned with the consolidated audit probe; see `docs/handoff/AFS-LOULAN-WEB-ROOT-LATEST-GATE-FACTS-PROJECT-AUDIT-SYNC-001.md`.

2026-06-01: Loulan project audit probe selected-file inspector now exposes a stable `latest_gate_facts` fact, letting the consolidated `afs_project_audit_package_probe.json` entry point show the current B01 gate blocker without adding a new UI fact per sync object; see `docs/handoff/AFS-LOULAN-WEB-PROJECT-AUDIT-LATEST-GATE-FACTS-DIRECT-001.md`.
...
2026-06-01: Memory Workbench now summarizes a selected Loulan manifest set as a project-level no-call cockpit when the full package is not selected, surfacing 9-manifest real-project coverage, 87 assets, 3 eligible refs, 84 blocked refs, 5 pending B01 decisions, 38 Image2 requests, 38 Kling I2V requests, and B02 blocked status without scanning directories, calling providers, promoting assets, or writing durable Memory; see `docs/handoff/AFS-LOULAN-WEB-MANIFEST-SET-SUMMARY-001.md`.
...
2026-06-01: Web Artifact Workspace now recognizes the Loulan `loulan_afs_project_audit_package_probe` / `afs_project_audit_package_probe.json` directly as a read-only memory artifact, surfacing project audit pass states plus the B01 operator entrypoint package-sync facts, package audit-summary sync counts, and CLI summary probe facts while keeping B01, promotion, provider, media, and durable Memory gates blocked; see `docs/handoff/AFS-LOULAN-WEB-PROJECT-AUDIT-PROBE-DIRECT-001.md`.

2026-06-01: Loulan memory package now surfaces the asset-governance phase audit as `project_audits.phase_gate`, and Web package review shows it in the Project audits card, protocol controls, and inspector facts; current no-call probe over `D:\Projects\LoulanSceneAssets` reports `blocked_until_b01_human_review`, 3 eligible refs, 90 blocked refs, no provider calls, and a clean safety scan; see `docs/handoff/AFS-LOULAN-PACKAGE-PHASE-GATE-AUDIT-001.md`.

2026-06-01: Loulan memory package now carries sanitized project-audit summary counts from the manifest-reference, text-encoding, and asset-governance phase audits; CLI stdout, package Markdown, and Web package review show manifest errors, invalid asset/status counts, text errors, phase failures, and pending B01 counts without raw audit body copy, provider calls, media copy, approval inference, or durable Memory writes; see `docs/handoff/AFS-LOULAN-PACKAGE-AUDIT-SUMMARY-001.md`.

2026-06-01: Web Artifact Workspace now recognizes the Loulan `loulan_asset_governance_phase_audit` / `asset_governance_phase_audit.json` directly as a read-only memory artifact, and the project-audit-probe inspector now surfaces `phase_gate_audit` when present; current selected-file probe shows 5 phases, 4 passed, 1 expected-blocked phase, 3 eligible refs, 84 registry-blocked refs, and 5 pending B01 decisions; see `docs/handoff/AFS-LOULAN-WEB-PHASE-AUDIT-DIRECT-001.md`.

2026-06-01: Web Artifact Workspace now recognizes Loulan governance audit source files `manifest_reference_audit.json` and `text_encoding_audit.json` directly as read-only memory artifacts; current selected-file probes show manifest audit `pass` with 14 JSON files / 87 registry assets / 0 invalid asset types / 0 invalid statuses and text audit `pass` with 268 text files / 0 issues, without provider calls, media writes, or durable Memory writes; see `docs/handoff/AFS-LOULAN-WEB-GOVERNANCE-AUDITS-DIRECT-001.md`.

2026-06-01: Web Artifact Workspace now recognizes the Loulan root `project_manifest.json` directly as `loulan_root_project_manifest`, showing 38 shots, 16:9 target format, package/governance audit statuses, the package audit-summary status chain, and B01/next-context blockers without package regeneration, provider calls, media writes, or durable Memory writes; see `docs/handoff/AFS-LOULAN-WEB-ROOT-PROJECT-MANIFEST-DIRECT-001.md`.
