...
from narratocut.harness.narratostudio_quality import build_narratostudio_review_section
from narratocut.harness.posterflow_quality import build_posterflow_review_section
...
def review_run(run_dir: str | Path) -> dict[str, Any]:
    ...
    if run_manifest and run_manifest.get("quality_profile") == NARRATOSTUDIO_PRODUCTION_HANDOFF_PROFILE:
        sections.append(build_narratostudio_review_section(root))
    if run_manifest and run_manifest.get("quality_profile") == POSTERFLOW_MEMORY_DEMO_PROFILE:
        sections.append(build_posterflow_review_section(root))
