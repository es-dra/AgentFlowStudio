...
from narratocut.harness.quality_profiles import NARRATOSTUDIO_PRODUCTION_HANDOFF_PROFILE
...
def narratostudio_artifacts_to_inspect() -> list[str]:
    return list(REQUIRED_ARTIFACTS)


def build_narratostudio_quality_report(root: str | Path) -> dict[str, Any]:
    run_dir = Path(root)
    checks: list[dict[str, Any]] = []
    artifacts = {name: _read_json_object(run_dir / name) for name in REQUIRED_ARTIFACTS if name.endswith(".json")}

    for artifact in REQUIRED_ARTIFACTS:
        _add_file_check(run_dir / artifact, f"narratostudio_{_check_name(artifact)}_exists", checks)
    ...
    return {
        ...
        "summary": {
            "quality_profile": NARRATOSTUDIO_PRODUCTION_HANDOFF_PROFILE,
...
def _add_json_parse_checks(run_dir: Path, checks: list[dict[str, Any]]) -> None:
    for artifact in REQUIRED_ARTIFACTS:
        ...
        _add_check(
            checks,
            f"narratostudio_{_check_name(artifact)}_json_valid",
            "pass" if _read_json_object(path) is not None else "fail",
        )


def build_narratostudio_review_section(root: str | Path) -> dict[str, Any]:
    report = build_narratostudio_quality_report(root)
    checks = [_review_check(check) for check in report["checks"]]
    return {
        "name": "narratostudio_artifacts",
...
def _add_schema_checks(artifacts: dict[str, dict[str, Any] | None], checks: list[dict[str, Any]]) -> None:
    for name, payload in artifacts.items():
        ...
        _add_check(
            checks,
            f"narratostudio_{_check_name(name)}_schema_version",
...
def _add_model_validation_check(name: str, payload: dict[str, Any], checks: list[dict[str, Any]]) -> None:
    ...
    try:
        ...
        _add_check(
            checks,
            f"narratostudio_{_check_name(name)}_schema_valid",
            "fail",
            {"error_count": len(exc.errors())},
        )
        return
    _add_check(checks, f"narratostudio_{_check_name(name)}_schema_valid", "pass")
...
def _add_auxiliary_checks(artifacts: dict[str, dict[str, Any] | None], checks: list[dict[str, Any]]) -> None:
    ...
    _add_check(checks, "narratostudio_memory_candidate_only", "pass" if statuses <= {"candidate"} else "fail")
    _add_check(
        checks,
        "narratostudio_cost_trace_local_deterministic",
        "pass"
    ...
    _add_check(
        checks,
        "narratostudio_feedback_signal_derived",
        "pass"
...
def _check_name(filename: str) -> str:
    return filename.replace(".json", "").replace(".md", "")
