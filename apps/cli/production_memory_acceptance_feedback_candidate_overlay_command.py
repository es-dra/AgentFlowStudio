...
def production_memory_loop_run_acceptance_feedback_candidate_reviewed_no_provider_command(
    ...
    promotion_decision_path: Path = typer.Option(
        ...
        dir_okay=False,
        readable=True,
        help="Path to acceptance feedback candidate promotion decision JSON.",
    ),
    output_dir: Path = typer.Option(
        Path("data/processed/runs/production_memory_loop/acceptance_feedback_candidate_reviewed"),
        "--output",
        "-o",
        help="Directory for acceptance feedback candidate reviewed run artifacts.",
    ),
) -> None:
    """Overlay reviewed acceptance feedback candidate and build no-provider context."""
    try:
        payload = load_production_memory_loop(loop_path)
        packet = load_acceptance_feedback_candidate_packet(candidate_packet_path)
        promotion_decision = load_acceptance_feedback_candidate_promotion_decision(promotion_decision_path)
        derived_loop, run, overlay = build_acceptance_feedback_candidate_reviewed_run(
            payload,
            packet,
            promotion_decision,
        )
        written_paths = write_acceptance_feedback_candidate_reviewed_run(derived_loop, run, overlay, output_dir)
    except ValueError as exc:
        typer.echo(f"Production memory acceptance feedback candidate reviewed run failed: {exc}", err=True)
        raise typer.Exit(code=1) from exc

    readiness = run["pass_readiness"]
    typer.echo(f"Production memory acceptance feedback candidate reviewed run: {'ready' if readiness['ready'] else 'blocked'}")
...
__all__ = ("production_memory_loop_run_acceptance_feedback_candidate_reviewed_no_provider_command",)
