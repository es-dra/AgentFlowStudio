...
def production_memory_loop_review_operator_feedback_candidate_command(
    ...
    output_dir: Path = typer.Option(
        Path("data/processed/runs/production_memory_loop/operator_feedback_candidate_promotion"),
        "--output",
        "-o",
        help="Directory for operator feedback candidate decision artifacts.",
    ),
) -> None:
    """Create an explicit decision for an operator feedback memory candidate."""
    try:
        packet = load_operator_feedback_candidate_packet(packet_path)
        ...
        written_paths = write_operator_feedback_candidate_promotion_decision(promotion_decision, output_dir)
    except ValueError as exc:
        typer.echo(f"Production memory operator feedback candidate decision failed: {exc}", err=True)
        raise typer.Exit(code=1) from exc

    reuse = "allowed" if promotion_decision["candidate_reuse_allowed"] else "blocked"
    typer.echo(f"Production memory operator feedback candidate decision: {promotion_decision['decision']}")
...
__all__ = ("production_memory_loop_review_operator_feedback_candidate_command",)
