...
def run_workflow(workflow_path: Path, input_path: Path, output_dir: Path | None = None) -> dict[str, Any]:
    ...
    run = WorkflowRunner(default_node_registry(), progress_callback=bridge_progress).run(workflow, context)
...
def _run_workflow_thread(workflow: Any, context: WorkflowContext, run_key: str) -> None:
    try:
        WorkflowRunner(default_node_registry(), progress_callback=bridge_progress).run(workflow, context)
