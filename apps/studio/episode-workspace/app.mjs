...
function renderDeliveryMode() {
  ...
  return `<section class="focused-mode delivery-mode"><header><span>交付状态</span><h2>${blockers.length ? "交付仍被真实条件阻断" : "交付事实已齐备"}</h2><p>${blockers.length ? blockers.map(blockerLabel).join("；") : "服务未提供 artifact availability proof，工作区不会伪造可播放性或允许冻结。"}</p></header><div class="delivery-sheet"><div><strong>${model.delivery.missing_asset_count ?? 0}</strong><span>已证实缺失</span></div><div><strong>${model.truth.generation_dispatch_count ?? 0}</strong><span>真实任务标识</span></div><div><strong>${model.delivery.playable_preview_available ? "可用" : "未证实"}</strong><span>可播放预览</span></div></div>${canUnlock ? '<button class="primary-button" data-command="unlock-delivery">重新打开交付版本</button>' : '<button class="primary-button" disabled>冻结交付版本</button>'}<p class="action-reason">冻结只在服务提供真实 locked selections、missing=0 与 artifact availability proof 后开放。</p></section>`;
}
