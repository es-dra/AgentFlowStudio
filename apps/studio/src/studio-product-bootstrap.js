...
const CANVAS_SHELL_HTML = `<main id="canvas-root"><div id="canvas-viewport"><div id="world"><svg id="edge-layer" xmlns="http://www.w3.org/2000/svg"></svg><div id="node-layer"></div></div></div><section id="canvas-empty-hint" hidden aria-label="空画布开始"><form class="canvas-empty-onboarding"><div class="empty-kicker">AFS 创作画布</div><h1 class="canvas-empty-title">从一个想法开始制作</h1><p class="canvas-empty-copy">画布现在是空的。先写下故事方向、导入剧本，或创建一个空白文本节点；确认前不会生成角色、场景或镜头。</p><textarea data-empty-action="idea-text" rows="4" maxlength="12000" placeholder="写下想法、故事梗概或一段剧本文字"></textarea><div class="canvas-empty-actions"><button type="submit" class="empty-primary-action">输入想法</button><button type="button" data-empty-action="import-script">导入剧本</button><button type="button" data-empty-action="blank-node">空白节点</button></div><dl class="canvas-empty-counts"><div><dt>节点</dt><dd>0</dd></div><div><dt>场景</dt><dd>0</dd></div><div><dt>镜头</dt><dd>0</dd></div></dl></form></section><div id="prompt-bar-layer"></div></main><div id="corner-controls" aria-label="画布视图控制"></div>`;
