...
import { visualAssetDefaultsFromAssetCardDraft } from "./visual-asset-draft-defaults.js";

export function visualAssetDefaults(node, imageAsset, assetType) {
  const draftDefaults = visualAssetDefaultsFromAssetCardDraft(node?.params?.assetCardDraft, assetType);
...
function sceneDefaults(node, text) {
  ...
  const card = {
    ...
    view_set: "同一场景的俯瞰全景、正向广角、入口/边缘视角、光影或材质细节视角保持一致",
  };
  return {
    label,
