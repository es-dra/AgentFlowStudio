...
def _compact_mode_value(value: Any) -> str:
    return str(value or "").replace(" ", "").replace("-", "_").casefold()


_NORMALIZED_ORIGINALIZE_VALUES = {
    _compact_mode_value(item) for item in _ORIGINALIZE_MODE_VALUES
}
_NORMALIZED_LOCALIZED_VALUES = {
    _compact_mode_value(item) for item in _LOCALIZED_MODE_VALUES
}
...
def normalize_reference_transform_mode(value: Any) -> str:
    ...
    if not text:
        return ""
    compact = _compact_mode_value(text)
    if compact in _NORMALIZED_ORIGINALIZE_VALUES:
        return ORIGINALIZE_REFERENCE_MODE
    if compact in _NORMALIZED_LOCALIZED_VALUES:
