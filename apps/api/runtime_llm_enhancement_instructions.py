...
def visual_enhancement_instruction(request: PromptOptimizationRequest) -> str:
    ...
    reference_role_line = (
        "参考图用途：先判断参考图角色。只有当原始提示词明确要求“这个人物/这张图主体/基于参考图编辑/保持同一人物”时，"
        "参考图才是主体参考；如果原始提示词明确指定新主体（例如狸花猫、某个新角色、某个新场景），参考图就是风格参考或视觉线索，"
        "不要把参考图主体替换用户指定主体，不要把参考图背景强行写成必须保留的场景。"
    )
    parts = [
        f"意图：围绕“{request.prompt_text}”完成本次生成。",
        "人物/主体：以原始提示词明确指定的主体为最高优先级；主体参考时保留参考图中的同一个人物，风格参考时只吸收质感、构图或画风，不改写主体身份。",
        "场景/美术：保持原提示中的场景信息；只有主体参考或用户明确要求延续场景时才保留参考图场景，未指定时不要新增具体地点。",
        f"动作/情节：只执行“{request.prompt_text}”这一项变化，不扩写新剧情。",
        "镜头/构图：关键帧清晰呈现主体变化，构图稳定，主体可辨识。",
        "灯光：保持自然可读的光线，不改变参考图的主要光感。",
        "运动/时间推进：单帧关键画面，不制造多阶段动作。",
        "连续性：主体参考时保持参考图人物身份、脸部辨识度、服装、体型比例和整体风格；风格参考时保持用户指定主体，只迁移画面质感和可读风格线索。",
