...
"""Best-effort lexical attribute vocabulary for visual-asset lock conflict hints.
...
# attribute -> value -> terms (zh + en). Values within one attribute are
# mutually exclusive; a lock pinning one value conflicts with a prompt
# asserting a different value of the same attribute.
ATTRIBUTE_GROUPS: dict[str, dict[str, tuple[str, ...]]] = {
    "hair_color": {
        # "re:" terms are regular expressions; en colors match within a
        # 2-word window before "hair" so "black short hair" still hits black.
        "black": (r"re:\bblack\b(?:\s+[\w'-]+){0,2}\s+hair\b", "黑发", "黑色头发", "黑色长发", "黑色短发"),
        "brown": (r"re:\bbrown\b(?:\s+[\w'-]+){0,2}\s+hair\b", "棕发", "棕色头发", "褐色头发"),
        "blonde": (r"re:\bblonde?\b(?:\s+[\w'-]+){0,2}\s+hair\b", "金发", "金色头发"),
        "red": (r"re:\bred\b(?:\s+[\w'-]+){0,2}\s+hair\b", "红发", "红色头发", "红色长发", "红色短发"),
    ...
    "outfit_color": {
        "black": ("black coat", "black dress", "black suit", "black jacket", "black shirt", "黑色外套", "黑色风衣", "黑色连衣裙", "黑色西装", "黑色衬衫", "黑衣"),
        "white": ("white coat", "white dress", "white suit", "white jacket", "white shirt", "白色外套", "白色风衣", "白色连衣裙", "白色西装", "白色衬衫", "白衣"),
        "red": ("red coat", "red dress", "red suit", "red jacket", "red shirt", "红色外套", "红色风衣", "红色连衣裙", "红色西装", "红色衬衫", "红衣"),
        "blue": ("blue coat", "blue dress", "blue suit", "blue jacket", "blue shirt", "蓝色外套", "蓝色风衣", "蓝色连衣裙", "蓝色西装", "蓝色衬衫", "蓝衣"),
        "green": ("green coat", "green dress", "green suit", "green jacket", "green shirt", "绿色外套", "绿色风衣", "绿色连衣裙", "绿色西装", "绿色衬衫"),
        "yellow": ("yellow coat", "yellow dress", "yellow jacket", "黄色外套", "黄色连衣裙", "黄衣"),
        "purple": ("purple coat", "purple dress", "purple jacket", "紫色外套", "紫色连衣裙"),
        "gray": ("gray coat", "grey coat", "gray suit", "灰色外套", "灰色西装", "灰衣"),
    },
    "build": {
        "slim": ("slim build", "slender", "瘦削", "纤细", "苗条"),
        "muscular": ("muscular", "肌肉发达", "健壮"),
        "heavy": ("heavyset", "stocky", "魁梧", "壮硕", "肥胖"),
        "petite": ("petite", "娇小"),
    },
    "facial_mark": {
        "scar": ("scar", "疤痕", "刀疤"),
...
def attribute_values_in_text(text: str) -> dict[str, set[str]]:
    """Return {attribute: {value, ...}} for every vocabulary term found in text."""
...
def find_lock_conflicts(lock_text: str, prompt_text: str) -> list[dict[str, Any]]:
    """Best-effort lexical conflicts between one lock line and the prompt.

    Low recall by design: only fires when the lock pins a vocabulary value
    and the prompt asserts a different value of the same attribute.
    """
