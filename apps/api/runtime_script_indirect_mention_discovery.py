"""Indirect-mention discovery (structural wide recall only).

Production note:
  Discovery itself is free/deterministic. Semantic judgment uses a *paid*
  remote LLM and lives in runtime_script_indirect_mention_proposals.py under
  AFS_ENABLE_INDIRECT_MENTION_LLM_PROPOSALS (default off). Do not confuse this
  path with free deterministic proposal flags (alias / scene-name).

Wide-recall rules:
  - NO left-context vocabulary whitelist
  - Quoted spans accepted with weak length/script checks
  - Cue/parent patterns are additional recall only
  - Open unquoted CJK NER intentionally disabled (~500–600 hits/long script)
"""
...
_QUOTE_NAME = re.compile(r"[「『\"“](?P<name>[^」』\"”\n]{2,24})[」』\"”]")
...
def discover_indirect_mention_candidates(
    source_text: str,
    *,
    known_identity_surfaces: set[str] | frozenset[str] | None = None,
) -> list[dict[str, Any]]:
    """Find suspected name mentions lacking label/speaker/bio evidence at that span.

    known_identity_surfaces: optional extra exact surfaces (canonical names and
    confirmed aliases from existing assets). Mentions matching these are marked
    already_extracted_as_character=true so the paid proposal builder can skip them.
    """
    evidenced = _evidenced_name_spans(source_text)
    extracted_names = {item.value for item in extract_characters(source_text)}
    known_names = set(extracted_names)
    ...
    for match in _QUOTE_NAME.finditer(source_text):
        hits.extend(_candidate_from_match(source_text, match, method="quoted_name", evidenced=evidenced))
    for match in _MENTION_CUE.finditer(source_text):
        hits.extend(_candidate_from_match(source_text, match, method="mention_cue", evidenced=evidenced))
    for match in _PARENT_NAME_CUE.finditer(source_text):
        hits.extend(_candidate_from_match(source_text, match, method="parent_name_cue", evidenced=evidenced))
    ...
    for hit in sorted(hits, key=lambda item: (item["start"], item["end"], item["mention"])):
        ...
        by_name[name] = {
            ...
            "occurrence_count": 1,
            "already_extracted_as_character": name in known_names,
            "source_span": {"start": hit["start"], "end": hit["end"], "quote": hit["quote"]},
        }
...
def _candidate_from_match(
    ...
    *,
    ...
    if method == "quoted_name":
        if not _looks_like_quoted_candidate(name):
            return []
    elif not _is_person_name(name):
        return []
...
def _looks_like_quoted_candidate(value: str) -> bool:
    text = value.strip()
